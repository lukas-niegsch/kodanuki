...
void TargetState::create_depth_image()
{
    ...
    allocate_info.memoryTypeIndex = find_memory_type(device, memory_requirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    CHECK_VULKAN(vkAllocateMemory(device, &allocate_info, nullptr, &depth_image_memory));
	...
	image_view_info.format = depth_image_format;
    ...
    CHECK_VULKAN(vkCreateImageView(device, &image_view_info, nullptr, &depth_image_view));
}
